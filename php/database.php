<?php 
  ...
  function db_selectColumns(string $table_name, array $columns, ?array $filters = null) {
   ...
   //    'Sex' => "1"
   // );
   // db_newUser($user, 'mescouilleslalalalalalalalal');
   ...
   // $filter = array( "UserID" => array("LIKE", "'U%'", "2"),
   //                  "Username" => array("LIKE", "'Jo%'", "0")
   // );
   // //java_log(json_encode(db_selectColumns("user", ["UserID", "Username"], $filter)));
   // var_dump(db_selectColumns("user", ["UserID", "Username"]));
   // java_log(json_encode(db_selectColumns("user", ["UserID", "Username"])));
?>
